...
const postPoll = async (pollObj) => {
  const resolved = await axios.post("http://localhost:3001/poll/createPoll",{pollObj: pollObj}).catch((err) => {
...
const putPoll = async (titleStr, pollIndex) => {
  ...
  let response = await axios.put(`http://localhost:3001/poll/vote/${titleStr}`, {pollIndex: pollIndex});
